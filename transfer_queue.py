"""
transfer_queue - the runtime that turns Mike's "transfer to a human" into
...
# ---------------------------------------------------------------------------
# /prepare — called by Mike right before SIP REFER
# ---------------------------------------------------------------------------
...
async def voice_webhook(
    ...
    that Mike prepared a moment ago, then return <Enqueue> TwiML.
    """
...
async def add_callback(request: Request) -> JSONResponse:
    """
    Add a callback row. Called by Mike's schedule_callback tool when the prospect
